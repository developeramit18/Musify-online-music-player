...
export default function EditPlaylist() {
    ...
    try {
        ...
        if(response.status === 200){
          setPlaylistData(prev => ({
            ...
            thumbnail: response.data.playlist.thumbnail,
            songs: response.data.playlist.songs.map((song) => song._id)
          }));
        }
    } catch (error) {
        toast.error(error.response.data.message || "Error fetching playlist data");
    }
  }
  ...
  const handlePlaylistPicUpload = async (folder) => {
    if (!playlistPic) {
      if (!toast.isActive(toastId)) {
        toast.error("Select playlist thumbnail first!", { toastId });
      }
      return;
    }
    try {
  ...
  const handleAddPlaylist = async() =>{
    ...
    try {
      setLoading(true);
      const response = await axios.put(`/api/playlist/${playlistId}`, playlistData);

      if(response.status === 200){
        toast.success(response.data.message || "Playlist updated successfully");
        setLoading(false);
        setPlaylistData(prev => ({
          ...prev,
          name: response.data.playlist.name,
  ...
  return (
    <div className="w-full h-full p-0 sm:p-2 flex flex-col">
      <h2 className="text-2xl font-bold text-center">Update Playlist</h2>
