...
export default function ArtistPlaylist() {
  ...
  const fetchAllDurations = async () => {
    if (artistData.songs) {
      const durationPromises = artistData.songs.map((song) =>
  ...
  const handlePlayPlaylist = () => {
    if (userState.isLoggedIn) {
      dispatch(addSongs([].concat(...artistData.songs)));
    } else {
      navigate("/signin");
    }
  };
      ...
      {loading ? (
        ...
        <div>
          <div className="relative w-full h-72">
            ...
            />
            <h1 className="text-2xl sm:text-3xl md:text-6xl font-bold text-white absolute left-4 top-1/2 -translate-y-1/2 z-10">
              {artistData.name}
            </h1>
          </div>
              ...
              {artistData.songs &&
                artistData.songs.map((song, index) => (
                  ...
                  >
                    ...
                    >
                      <span className="hidden md:flex group-hover:hidden text-md font-semibold w-2 md:w-3">
                        {index + 1}.
                      </span>
                      <FaPlay
                        title={`Play - ${song.title}`}
                        className="flex md:hidden group-hover:flex text-xs w-2 md:w-3"
                      />
                      <img
                        onContextMenu={(e) => e.preventDefault()}
                        src={song.thumbnail}
